...
class CropFactPhysiologySchema(DataProductSchema):
    ...
    def create_query_template(self):
        ...
        query_template = f"""
                ...
                (SELECT (s.temporal_dimension -> {self.end_year_index()} ->> 'value')::integer
                FROM (SELECT s.temporal_dimension, row_number() OVER (ORDER BY (s.temporal_dimension -> {self.end_year_index()} ->> 'value')::integer DESC) AS row_num
                FROM datafactory.statistics s
                WHERE s.job_id = '{self.job_id}' and d.dataset_id = '{self.dataset_id}') sub_query
                WHERE row_num = 1) AS end_year
