...
class CropFactPhysiologyProvider:
        ...
        for response, product_dimension, dataset_list in zip(api_response, product_dimensions, dataset_filtered_list):
            if not response:
                ...
                for gs in growth_stages:
                    ...
                    if filter_df['growth_stage'].values[0]:
                        ...
                        temporal_dimension_data.extend([{"type": "START_DATE", "value": None},
                                                        {"type": "END_DATE", "value": None},
                                                        {"type": "END_YEAR", "value": None}])
                    ...
                    if not filter_df['growth_stage'].values[0]:
                        df_str_value = (filter_df[filter_df['Texture_text'].notnull() & filter_df['Texture_text'].apply(lambda x: type(x) == str)])
                        feature_properties['Texture_text'] = str(df_str_value['Texture_text'].values[0])
                    properties.update(temporal_dimension=temporal_dimension_data, spatial_dimension=spatial_dimension,
                                      product_dimension=product_dimension,dataset_list = dataset_list)
    ...
    def json_unpack(self, response_obj):
        ...
        for i, row in df.iterrows():
            ...
            elif not growth_stages and values:
                data = {}
                data[f"{name}_{code}"] = values
                data['growth_stage'] = growth_stages
                data.update(self.geom)
                self.nested_json.append(data)
